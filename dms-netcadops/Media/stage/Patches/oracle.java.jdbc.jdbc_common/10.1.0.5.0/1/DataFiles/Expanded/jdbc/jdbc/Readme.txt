...
*   In the section for "Shortcuts For Inserting and Retrieving CLOB Data"
    in the 10g JDBC manual there is reference to a connection property
    "SetBigStringUseClob" which is incorrect. Please use the correct 
    property name which is "SetBigStringTryClob"

*   Using oracle.sql.TIMESTAMPTZ with the JDBC OCI driver in a 
    multi-threaded setting might cause Java core dump, because of a JDK
    bug.  This happens only on 64-bit Sparc machines, when using the 64-bit
    version of Java.  The workaround is to use different connections
    per thread, and to avoid sharing any timezone info
    among threads when creating the oracle.sql.TIMESTAMPTZ objects.
    Please refer to "(Sun's Review Id: 234589)"
